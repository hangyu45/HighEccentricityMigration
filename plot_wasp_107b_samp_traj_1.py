...
"""
Reproducing fig. 1 of 2406.00187.
Because octLK is chaotic, the final result may differ from machine to machine.
...
def plot_traj_from_file(fid, grp_id, fig_file):
    ...
    R2_eq = np.zeros(len(tt))
    c_iS = np.zeros(len(tt))
    for i in range(len(R2)):
        ...
        _Li_v = sol.y[:3, i]
        _S1_v = sol.y[12:15, i]
        c_iS[i] = LK.inner(_Li_v, _S1_v)/np.sqrt(LK.inner(_Li_v, _Li_v) * LK.inner(_S1_v, _S1_v))
    
    print(np.arccos(c_iS[-1])*180./np.pi)

    fig=plt.figure(figsize=(9, 27.5))
...
#     ax.axhline(0.02, color='tab:red', ls=':')
    ...
    ax.plot(tt/P_yr/1e6, np.arccos(c_iS)*180./np.pi, 
            color='C7', alpha=0.6)
    ax.set_ylabel(r'$I_{iS}$ [$^\circ$]')
...
if __name__=='__main__':    
    ...
    parser.add_argument('--grp-id', type=str, default=0)
    parser.add_argument('--fig-file', type=str, default='wasp_107b_samp_1.png')
